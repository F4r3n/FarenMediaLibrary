...
namespace pfd
{
	...
	namespace internal
	{
		...
		static bool starts_with(std::string const &str, std::string const &prefix)
		{
			...
			std::string executor::result(int *exit_code)
			{
				stop();
				if (exit_code)
...
#if _WIN32
			void executor::start(std::function<std::string(int *)> const &fun)
			{
				stop();
			...
			void executor::start(std::string const &command)
			{
				stop();
				m_stdout.clear();
				m_exit_code = -1;
			...
			bool executor::ready(int timeout)
			{
				if (!m_running)
					return true;
...
#if _WIN32
		platform::dll::dll(std::string const &name): handle(::LoadLibraryA(name.c_str()))
		{}
		...
		platform::new_style_context::new_style_context()
		{
			// Only create one activation context for the whole app lifetime.
			static HANDLE hctx = platform::new_style_context::create();
		...
		HANDLE platform::new_style_context::create()
		{
			...
			platform::dll comdlg32("comdlg32.dll");
...
#if !_WIN32
			std::string dialog::desktop_helper() const
			{
#if __APPLE__
				return "osascript";
#else
			...
			std::string dialog::buttons_to_name(choice choice) const
			{
				switch (choice)
				{
			...
			std::string dialog::get_icon_name(icon icon) const
			{
				switch (icon)
				{
			...
			std::string dialog::powershell_quote(std::string const &str) const
			{
				return "'" + std::regex_replace(str, std::regex("['\"]"), "$&$&") + "'";
			}

			// Properly quote a string for osascript: replace ' with '\'' and \ or " with \\ or \"
			std::string dialog::osascript_quote(std::string const &str) const
			{
			...
			std::string dialog::shell_quote(std::string const &str) const
			{
				return "'" + std::regex_replace(str, std::regex("'"), "'\\''") + "'";
			}

			// Check whether a program is present using “which”
			bool dialog::check_program(std::string const &program)
			{
#if _WIN32
				(void)program;
				return false;
			...
			file_dialog::file_dialog(type in_type,
			...
			std::string file_dialog::string_result()
			{
#if _WIN32
			...
			std::vector<std::string> file_dialog::vector_result()
			{
#if _WIN32
				m_async->result();
			...
			std::string file_dialog::select_folder_vista(IFileDialog *ifd)
			{
				std::string result;
