...
namespace fms
{
	...
	void OGLRenderingSystem::update(float, EntityManager& em, EventManager&)
	{
		...
		for (auto&& e : em.iterate<fmc::CCamera>(fm::IsEntityActive))
		{
			...
			fm::OGLRenderer::getInstance().postProcess(_graphics, cam->GetRenderTexture().GetColorBufferTexture(0));
			...
			while (!cmdBuffers.empty())
			{
				...
				if (cmd._command == fm::Command::COMMAND_KIND::BLIT)
				{
					...
					if (cmd._source.kind == fm::Command::TEXTURE_KIND::TEXTURE)
					{
						...
						{
							std::vector<fm::OGLTexture> textures = currentCamera->GetRenderTexture().GetColorBuffer();
		...
		std::shared_ptr<fm::Material> inMaterial, fm::MaterialProperties* inMaterialProperties)
	{
		...
		if (shader != nullptr)
		{
			...
			cam->GetRendererConfig().uboLight->Bind();
			std::dynamic_pointer_cast<fm::OGLShader>(shader)->SetUniformBuffer("PointLights", cam->GetRendererConfig().uboLight->GetBindingPoint());
	...
	void OGLRenderingSystem::_Draw(fmc::CCamera* cam)
	{
		...
		//int i = 0;
	/*    while (!cam->GetRendererConfig().queue.Empty())
		{
			const fm::RenderNode node = cam->GetRendererConfig().queue.getFrontElement();
			...
			cam->GetRendererConfig().queuePreviousValue = state;
			cam->GetRendererConfig().queue.next();
