...
void PhysicSystem::update(float dt, EntityManager& em, EventManager& event) {
    ...
    for(auto e : em.iterate<fmc::CTransform, fmc::Body2D>()) {
        ...
        transform->position = fm::Vector2f(body->body->GetPosition().x - (transform->scale.x/2)*P2M, 
        body->body->GetPosition().y - (transform->scale.y/2)*P2M)*M2P;
...
void PhysicSystem::init(EntityManager& em, EventManager& event) {
    ...
    for(auto e : em.iterate<fmc::CTransform, fmc::Body2D>()) {
        ...
        fmc::CTransform* transform = e->get<fmc::CTransform>();
        body->identity = &e->ID;
        body->bodyDef.position.Set((transform->position.x + transform->scale.x/2)*P2M, (transform->position.y + transform->scale.y/2)*P2M);
