...
using namespace fm;
namespace rendering {
void StandardShapes::AddVertex(MeshContainer* mesh, const fm::math::Vector2f& position, const fm::math::Vector2f& uv) {
    mesh->vertices.push_back({ position, uv });
}

MeshContainer* StandardShapes::CreateQuad() {
    MeshContainer* meshContainer = new MeshContainer();
    AddVertex(meshContainer) AddVertex(meshContainer, { 0, 1 }, { 0.0, 1.0 });
